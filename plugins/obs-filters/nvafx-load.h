...
static void nvafx_get_sdk_path(char buffer[MAX_PATH])
{
	char value[MAX_PATH];
	PVOID pvData = value;
	...
	LSTATUS status = RegGetValue(
		...
		&BufferSize);
	wcstombs(buffer, (LPCWSTR)pvData, MAX_PATH);
	if (status != ERROR_SUCCESS) {
		printf("[noise suppress: NVAFX redist is not installed]");
		return NULL;
	} else {
		printf("[noise suppress: NVAFX SDK redist path is %s]", buffer);
	}
}
...
static bool load_lib(void)
{
	...
	nvafx_get_sdk_path(buffer);
	size_t length = strlen(buffer);
	wchar_t path[MAX_PATH];
	mbstowcs_s(&length, path, length + 1, buffer, length);
